...
int main(){
    ...
    while(1){
        ...
        //输出售货机状态
        system("cls");
        printf("***********自动售货机***********\n注:?代表该货道上没有货物，输入BACK进行撤回操作，最多撤回3步\n");
        for (int i=0;i<5;i++){
        ...
        if(strcmp("BACK",pro_inp[0])==0){
            if(pro_inp[1][0]==0){
                if(hvend[0].state==0){
                    ...
                    for(int i=0;i<5;i++)
                        vend[i]=hvend[0].vendm[i];
                    state=hvend[0].state;
                    total_price=hvend[0].total_price;
                    hvend[0]=hvend[1];
                    hvend[1]=hvend[2];
                    hvend[2].state=0;
                    fseek(ptr,0,SEEK_SET);
...
void add_history(his_vendm vendm[3],Aisles vend[5],int state,int price){
    ...
    vendm[1]=vendm[0];
    vendm[0].state=state;
    vendm[0].total_price=price;
    vendm[0].vendm[0]=vend[0];
    vendm[0].vendm[1]=vend[1];
    vendm[0].vendm[2]=vend[2];
    vendm[0].vendm[3]=vend[3];
    vendm[0].vendm[4]=vend[4];
}